...
export function MultiSplitTransactionEditor({
  ...
  if (groupId && query.isLoading) return <div className="py-6 text-center text-[12px] text-[var(--text-secondary)] ">加载交易详情…</div>
  if (groupId && query.isError) return <div className="flex items-center justify-between py-4 text-[12px] text-[var(--danger)] "><span>交易详情加载失败</span><button type="button" onClick={() => void query.refetch()} style={{ color: 'var(--brand)' }}>重试</button></div>

  const pending = mutation.isPending || createMutation.isPending
  const minimumSplits = groupId ? 1 : 2

  return <div className="flex flex-col gap-3">
    <div className="flex gap-1" role="tablist" aria-label="拆分交易类型">{(['withdrawal', 'deposit', 'transfer'] as const).map((type) => <button key={type} type="button" role="tab" aria-selected={groupType === type} onClick={() => changeType(type)} className="flex-1 rounded-[5px] px-2 py-1.5 text-[12px]" style={{ background: groupType === type ? 'var(--brand)' : 'var(--surface-hover)', color: groupType === type ? 'var(--brand-on)' : 'var(--text-secondary)' }}>{type === 'withdrawal' ? '支出' : type === 'deposit' ? '收入' : '转账'}</button>)}</div>
    ...
    {drafts.map((draft, index) => <section key={draft.clientId} className="flex flex-col gap-2 border-b pb-3 border-[var(--border-subtle)] ">
      <div className="flex items-center justify-between"><span className="font-mono tabular-nums text-[11px] text-[var(--text-secondary)] ">拆分 {index + 1}</span><button type="button" title="删除拆分" aria-label={`删除拆分 ${index + 1}`} disabled={drafts.length <= minimumSplits} onClick={() => removeDraft(draft.clientId)} className="rounded p-1 disabled:opacity-30 text-[var(--danger)] "><TrashIcon aria-hidden className="size-3.5" /></button></div>
      <div className="grid grid-cols-[120px_1fr] gap-2"><input inputMode="decimal" aria-label={`拆分 ${index + 1} 金额`} value={draft.amount} onChange={(event) => update(index, { amount: event.target.value.replace(/[^0-9.]/g, '') })} className="font-mono tabular-nums rounded-[5px] px-2 py-1.5 text-right" style={inputStyle} /><input aria-label={`拆分 ${index + 1} 描述`} value={draft.description} onChange={(event) => update(index, { description: event.target.value })} className="rounded-[5px] px-2 py-1.5" style={inputStyle} /></div>
    ...
    <div className="flex items-center justify-between"><button type="button" onClick={addDraft} className="flex items-center gap-1 rounded-[5px] px-2 py-1.5 text-[12px] text-[var(--brand)] "><PlusIcon aria-hidden className="size-3.5" />添加拆分</button><button type="button" disabled={pending || drafts.length < minimumSplits} onClick={() => void save()} className="rounded-[6px] px-3 py-1.5 text-[12.5px] disabled:opacity-50 bg-[var(--brand)] text-white font-semibold shadow-sm hover:bg-[var(--brand-hover)]">{pending ? '保存中…' : groupId ? '保存全部拆分' : '创建多拆分交易'}</button></div>
  </div>
}
