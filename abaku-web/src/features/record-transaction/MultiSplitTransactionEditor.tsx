...
export function MultiSplitTransactionEditor({
  ...
  if (groupId && query.isError) {
    return <InlineError message="交易详情加载失败" onRetry={() => void query.refetch()} />
  }

  const pending = mutation.isPending || createMutation.isPending
  const minimumSplits = groupId ? 1 : 2
  const enabledCurrencies = (currencies.data?.data ?? []).filter((item) => item.attributes.enabled !== false)
    ...
    <div className="flex flex-col gap-3">
      ...
      {drafts.map((draft, index) => (
        ...
        <fieldset key={draft.clientId} className="relative flex min-w-0 flex-col gap-2 border-b border-[var(--border-subtle)] pb-3">
          <legend className="font-mono text-[11px] font-semibold tabular-nums text-[var(--text-secondary)]">
            拆分 {index + 1}
          </legend>
          <IconButton
            label={`删除拆分 ${index + 1}`}
          ...
          <div className={ROW}>
            {groupType === 'deposit' ? (
              ...
              <Field label="来源账户">
                <AccountSelect value={draft.sourceId} accounts={assetAccounts} onChange={(sourceId) => update(index, { sourceId })} />
              </Field>
            )}
            {groupType === 'withdrawal' ? (
              ...
              <Field label="目标账户">
                <AccountSelect value={draft.destinationId} accounts={assetAccounts} onChange={(destinationId) => update(index, { destinationId })} />
              </Field>
            )}
          </div>
          ...
          <div className={ROW}>
            ...
            <Field label="币种">
              <Select
                ...
                onChange={(event) => {
                  const currency = enabledCurrencies.find((item) => item.id === event.target.value)
                  update(index, { currencyId: currency?.id, currencyCode: currency?.attributes.code })
                }}
              >
                <option value="">沿用账户币种</option>
                {enabledCurrencies.map((item) => (
                  <option key={item.id} value={item.id}>{item.attributes.code} · {item.attributes.name}</option>
                ))}
              </Select>
            </Field>
          </div>
          ...
          <div className={ROW}>
            <Field label="分类">
              <Input
                ...
                onChange={(event) => update(index, { category: event.target.value, categoryId: undefined })}
              />
            </Field>
            <Field label="预算">
              <Select
          ...
          <div className={ROW}>
            <Field label="外币">
              <Select
                value={draft.foreignCurrencyId ?? ''}
                onChange={(event) => {
                  const currency = enabledCurrencies.find((item) => item.id === event.target.value)
                  update(index, {
                    foreignCurrencyId: currency?.id,
                    foreignCurrencyCode: currency?.attributes.code,
                    foreignAmount: currency ? draft.foreignAmount : '',
                  })
                }}
              >
                <option value="">无外币金额</option>
                {enabledCurrencies.filter((item) => item.id !== draft.currencyId).map((item) => (
                  <option key={item.id} value={item.id}>{item.attributes.code} · {item.attributes.name}</option>
                ))}
              </Select>
            </Field>
            <Field label="外币金额" hint={draft.foreignCurrencyId ? undefined : '先选外币'}>
              <Input
                inputMode="decimal"
                disabled={!draft.foreignCurrencyId}
                placeholder="0.00"
                className="text-right font-mono tabular-nums"
                value={draft.foreignAmount ?? ''}
                onChange={(event) => update(index, { foreignAmount: event.target.value.replace(/[^0-9.]/g, '') })}
              />
            </Field>
          </div>
