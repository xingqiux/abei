...
vi.mock('../../components/abaku/BalanceAreaChart', () => ({ BalanceAreaChart: () => null }))
vi.mock('../../components/abaku/TransactionRow', () => ({ TransactionRow: () => null }))
...
beforeEach(() => {
  ...
  // 全时段笔数：GET /api/v1/accounts/7/transactions
  mocks.fireflyFetch.mockReset().mockResolvedValue({ data: [], meta: { pagination: { total: 42 } } })
...
describe('AccountDetailPage 确认删除', () => {
  it('确认后走 DELETE /api/v1/accounts/{id}，不是归档的 PUT', async () => {
    renderPage()
    await screen.findByRole('heading', { name: '招行储蓄卡' })
    ...
    await waitFor(() => expect(mocks.fireflyDelete).toHaveBeenCalledWith('/api/v1/accounts/7'))
    // 删除是删除，归档是归档，这里绝不能退化成 PUT active=false
    expect(mocks.fireflyPut).not.toHaveBeenCalled()
  ...
  it('删除失败时留在原地报错，不跳走也不谎报成功', async () => {
    mocks.fireflyDelete.mockReset().mockRejectedValue(new Error('账户下有未结算交易'))
    renderPage()
    await screen.findByRole('heading', { name: '招行储蓄卡' })
