    ...
    >
      {segments.map((segment) => (
        <Radio
          key={segment.value}
          value={segment.value}
          className="flex-1 cursor-pointer rounded-md px-3 py-1.5 text-center text-sm font-medium text-[var(--text-secondary)] transition-colors hover:text-[var(--text-primary)] data-checked:bg-[var(--brand)] data-checked:font-semibold data-checked:text-[var(--brand-on)]"
        >
          {segment.label}
        </Radio>
      ))}
