        ...
        <TabList aria-label={ariaLabel} className="-mb-px flex gap-6">
          {tabs.map((tab) => (
            ...
            >
              {tab.label}
              {tab.count != null && (
                <span className="rounded-full bg-[var(--surface-hover)] px-1.5 py-0.5 text-xs tabular-nums text-[var(--text-secondary)] group-data-selected:bg-[var(--brand-soft)] group-data-selected:text-[var(--brand-text)]">
                  {tab.count}
                </span>
              )}
            </Tab>
