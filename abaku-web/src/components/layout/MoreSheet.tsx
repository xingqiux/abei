...
export function MoreSheet() {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] md:hidden" role="presentation">
      ...
      >
        <div className="mx-auto mt-3 h-1 w-9 rounded-full bg-[var(--surface-selected)] " />
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.to}
                onClick={close}
                className="flex items-center justify-between rounded-md px-3 py-3 text-sm font-medium text-[var(--text-primary)] hover:bg-[var(--surface-hover)]  "
              >
