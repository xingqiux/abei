...
describe('fireflyFetch', () => {
  it('preserves field validation errors', async () => {
    ...
    await expect(fireflyFetch('/api/v1/test')).rejects.toMatchObject({
      status: 422,
  ...
  it('discards a response that belongs to a previous token', async () => {
    ...
    const pending = fireflyFetch('/api/v1/accounts')
    setStoredToken('user-b')
    resolveResponse(new Response(JSON.stringify({ data: [] }), { status: 200 }))

    await expect(pending).rejects.toBeInstanceOf(FireflySessionChangedError)
  })
  ...
  it('parses an unquoted download filename', async () => {
    ...
    const { fireflyDownload } = await import('./client')
    const result = await fireflyDownload('/api/v1/data/export/transactions?type=csv')
