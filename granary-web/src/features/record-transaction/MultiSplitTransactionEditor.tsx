...
export function MultiSplitTransactionEditor({
  ...
  if (groupId && query.isError) return <div className="flex items-center justify-between py-4 text-[12px] text-red-600 dark:text-red-400"><span>交易详情加载失败</span><button type="button" onClick={() => void query.refetch()} style={{ color: 'light-dark(var(--color-indigo-600), var(--color-indigo-500))' }}>重试</button></div>

  const pending = mutation.isPending || createMutation.isPending
  const minimumSplits = groupId ? 1 : 2

  return <div className="flex flex-col gap-3">
    <div className="flex gap-1" role="tablist" aria-label="拆分交易类型">{(['withdrawal', 'deposit', 'transfer'] as const).map((type) => <button key={type} type="button" role="tab" aria-selected={groupType === type} onClick={() => changeType(type)} className="flex-1 rounded-[5px] px-2 py-1.5 text-[12px]" style={{ background: groupType === type ? 'light-dark(var(--color-indigo-600), var(--color-indigo-500))' : 'light-dark(var(--color-gray-100), var(--color-gray-700))', color: groupType === type ? 'var(--color-white)' : 'light-dark(var(--color-gray-500), var(--color-gray-400))' }}>{type === 'withdrawal' ? '支出' : type === 'deposit' ? '收入' : '转账'}</button>)}</div>
