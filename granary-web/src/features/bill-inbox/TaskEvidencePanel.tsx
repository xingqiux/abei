  ...
  return (
    <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
      <section className="min-w-0">
        <h3 className="mb-2 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>REVIEW</h3>
        <div className="flex flex-wrap gap-1.5">
          ...
          {review.isLoading && <span className="text-[11.5px]" style={{ color: 'var(--g-ink-2)' }}>加载中…</span>}
        </div>
          ...
          {issueGroups.flatMap((group) => group.items.map((candidate) => (
            <div key={`${group.label}-${candidate.row_id}`} className="flex items-start gap-1.5 rounded-[4px] px-1.5 py-1" style={{ background: 'var(--g-surface-2)' }}>
              <div className="min-w-0 flex-1 text-[11px] leading-relaxed">
                <div className="truncate" style={{ color: 'var(--g-ink)' }}>
              ...
              <button type="button" title="定位并修复流水" aria-label={`定位流水 ${candidate.row_id}`} onClick={() => onReviewRow(candidate.row_id)} className="shrink-0 rounded p-1" style={{ color: 'var(--g-accent)' }}><LocateFixed size={12} /></button>
            </div>
          )))}
          {review.isSuccess && issueGroups.length === 0 && <span className="text-[11px]" style={{ color: 'var(--g-ink-2)' }}>没有需要人工修复的问题</span>}
        </div>
      </section>

      <section className="min-w-0">
        <h3 className="mb-2 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>产物</h3>
          ...
          {(artifacts.data?.data ?? []).map((artifact) => (
            <div key={artifact.id} className="flex items-start justify-between gap-2 text-[11.5px]">
              <div className="min-w-0 flex-1">
                <div className="truncate" style={{ color: 'var(--g-ink)' }}>{artifact.attributes.filename ?? `${artifact.attributes.kind}-${artifact.id}`}</div>
                <div className="truncate text-[10.5px]" style={{ color: 'var(--g-ink-2)' }}>{artifact.attributes.mime_type} · {formatBytes(artifact.attributes.size)} · {STAGE_LABELS[artifact.attributes.generation_stage]}{artifact.attributes.encrypted ? ' · 已加密' : ''}</div>
              </div>
              <button type="button" disabled={downloadingId !== null} title="下载产物" aria-label={`下载 ${artifact.attributes.filename ?? artifact.id}`} onClick={() => void download(artifact.id, artifact.attributes.filename ?? `artifact-${artifact.id}`)} className="shrink-0 rounded p-1 disabled:opacity-40" style={{ color: 'var(--g-accent)' }}>
                <Download size={13} />
              </button>
            </div>
          ))}
          {artifacts.isSuccess && artifacts.data.data.length === 0 && <span className="text-[11.5px]" style={{ color: 'var(--g-ink-2)' }}>无可下载产物</span>}
        </div>
      </section>

      <section className="min-w-0">
        <h3 className="mb-2 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>事件</h3>
        <div className="flex max-h-[140px] flex-col gap-1 overflow-y-auto">
          {(events.data?.data ?? []).map((event) => (
            <div key={event.id} className="text-[11.5px] leading-relaxed">
              <span className="font-num" style={{ color: 'var(--g-ink-2)' }}>{event.attributes.created_at ? formatDateTime(event.attributes.created_at) : '--'}</span>{' '}
          ...
          {events.isSuccess && events.data.data.length === 0 && <span className="text-[11.5px]" style={{ color: 'var(--g-ink-2)' }}>暂无事件</span>}
        </div>
      </section>

      {hasError && (
        <div className="lg:col-span-3 flex items-center justify-between text-[11.5px]" style={{ color: 'var(--g-danger)' }}>
