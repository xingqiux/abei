...
export function IgnoreConfirmDialog({
  ...
  return (
      ...
      footer={
        <>
          ...
          <button
            ...
            style={{ background: 'var(--g-danger)', color: '#fff', fontWeight: 'var(--g-weight-demibold)' }}
          >
    ...
    >
      <div>
        确认忽略任务「<span style={{ fontWeight: 'var(--g-weight-demibold)' }}>{title}</span>」？该任务下{' '}
        <span className="font-num">{total}</span> 条流水将不再出现在待处理列表中，此操作不可撤销。
      </div>
    </Modal>
  )
}
