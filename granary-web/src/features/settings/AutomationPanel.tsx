...
function RuleRow({ title, active, count, execution, pending, onTest, onTrigger }: { title: string; active?: boolean; count?: number; execution?: ExecutionResult; pending: boolean; onTest: () => void; onTrigger: () => void }) {
  return <div className="flex min-h-8 items-center gap-2 rounded-[4px] px-2" style={{ background: 'light-dark(var(--color-gray-100), var(--color-gray-700))' }}><span className="min-w-0 flex-1 truncate text-[12.5px]" style={{ color: active === false ? 'light-dark(var(--color-gray-500), var(--color-gray-400))' : 'light-dark(var(--color-gray-900), var(--color-gray-100))' }}>{title}</span>{count !== undefined && <span className="font-mono tabular-nums text-[11px] text-gray-500 dark:text-gray-400">匹配 {count}</span>}{execution && <span className="font-mono tabular-nums text-[11px]" title={`${execution.range.start} 至 ${execution.range.end}`} style={{ color: 'light-dark(var(--color-emerald-600), var(--color-emerald-400))' }}>已执行 {execution.count} · {formatDateTime(execution.completedAt)}</span>}<button type="button" title="测试规则" aria-label={`测试 ${title}`} disabled={active === false || pending} onClick={onTest} className="rounded p-1 disabled:opacity-30 text-gray-500 dark:text-gray-400"><SparklesIcon aria-hidden className="size-3.5" /></button><button type="button" title="执行规则" aria-label={`执行 ${title}`} disabled={active === false || pending || count === undefined} onClick={onTrigger} className="rounded p-1 disabled:opacity-30 text-indigo-600 dark:text-indigo-400"><PlayIcon aria-hidden className="size-3.5" /></button></div>
}
...
function RecurrenceRow({ recurrence, result, pending, onTrigger }: { recurrence: Recurrence; result?: RecurrenceResult; pending: boolean; onTrigger: () => void }) {
  ...
  return (
    <div className="rounded-[4px] px-2 py-1.5" style={{ background: 'light-dark(var(--color-gray-100), var(--color-gray-700))' }}>
      <div className="flex min-h-6 items-center gap-2">
        <span className="min-w-0 flex-1 truncate text-[12.5px]" style={{ color: attributes.active === false ? 'light-dark(var(--color-gray-500), var(--color-gray-400))' : 'light-dark(var(--color-gray-900), var(--color-gray-100))' }}>{attributes.title}</span>
      ...
      <div className="font-mono tabular-nums flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-gray-500 dark:text-gray-400">
        <span>最近 {executionLabel}</span>
        <span>下次 {nextLabel}</span>
        {result && <span style={{ color: 'light-dark(var(--color-emerald-600), var(--color-emerald-400))' }}>本次生成 {result.ids.length} 个 · {formatDateTime(result.completedAt)}</span>}
