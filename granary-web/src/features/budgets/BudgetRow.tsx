...
export function BudgetRow({ budget, limits, range, limitsLoading = false, limitsError = false, onRetryLimits }: { budget: Budget; limits: BudgetLimitInfo[]; range: DateRange; limitsLoading?: boolean; limitsError?: boolean; onRetryLimits?: () => void }) {
  ...
  return <>
    <div className="group flex min-h-8 flex-wrap items-center gap-2 rounded-[4px] px-2 py-1 text-[12.5px] hover:bg-[var(--g-surface-2)] sm:flex-nowrap sm:gap-3">
      <div className="min-w-0 flex-1 truncate" style={{ color: 'var(--g-ink)' }}>{attrs.name}</div>
      {limitsLoading ? <div className="order-2 w-full text-left text-[11px] sm:order-none sm:w-[350px] sm:text-right" style={{ color: 'var(--g-ink-2)' }}>限额加载中…</div> : limitsError ? <div className="order-2 flex w-full items-center gap-2 text-[11px] sm:order-none sm:w-[350px] sm:justify-end" style={{ color: 'var(--g-danger)' }}><span>限额加载失败</span><button type="button" onClick={onRetryLimits} style={{ color: 'var(--g-accent)' }}>重试</button></div> : <div className="order-2 flex w-full shrink-0 flex-col gap-1 sm:order-none sm:w-[350px]">{currencySummaries.map((summary) => <div key={summary.code || summary.symbol} className="flex min-w-0 items-center gap-3"><div className="flex min-w-[90px] flex-1 items-center sm:w-[150px] sm:flex-none">{summary.hasLimit ? <ProgressBar pct={summary.pct} colorVar={summary.over ? 'var(--g-danger)' : 'var(--g-accent)'} /> : <span className="text-[11px]" style={{ color: 'var(--g-ink-2)' }}>未设限额</span>}</div><div className="font-num min-w-0 flex-1 text-right sm:w-[187px] sm:flex-none" title={summary.code} style={{ color: summary.over ? 'var(--g-danger)' : 'var(--g-ink)' }}>{summary.symbol}{formatAmount(summary.spent)}{summary.hasLimit && <span style={{ color: 'var(--g-ink-2)' }}> / {summary.symbol}{formatAmount(summary.total)}</span>}</div></div>)}</div>}
      <button type="button" title="管理限额" aria-label={`管理 ${attrs.name} 的限额`} disabled={limitsLoading || limitsError} onClick={() => setLimitsOpen(true)} className="rounded p-1 sm:opacity-0 sm:group-hover:opacity-100 sm:focus:opacity-100 disabled:opacity-20" style={{ color: 'var(--g-ink-2)' }}><SlidersHorizontal size={13} /></button>
      <button type="button" title="编辑预算" aria-label={`编辑预算 ${attrs.name}`} onClick={openEdit} className="rounded p-1 sm:opacity-0 sm:group-hover:opacity-100 sm:focus:opacity-100" style={{ color: 'var(--g-ink-2)' }}><Pencil size={13} /></button>
    </div>
    <Modal open={limitsOpen} onClose={() => setLimitsOpen(false)} title={`${attrs.name} · 限额`} width={620} footer={<button type="button" onClick={() => setLimitsOpen(false)} className="rounded-[6px] px-3 py-1.5 text-[12.5px]" style={{ background: 'var(--g-surface-2)', color: 'var(--g-ink)' }}>完成</button>}>
      <div className="flex flex-col gap-3">
        {limits.map((limit, index) => <div key={limit.limitId} className="grid grid-cols-1 items-end gap-2 sm:grid-cols-[1fr_1fr_110px_60px]"><label className="flex min-w-0 flex-col gap-1 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>开始<input type="date" aria-label={`限额 ${index + 1} 开始`} value={drafts[limit.limitId]?.start ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [limit.limitId]: { ...current[limit.limitId], start: event.target.value } }))} className="font-num min-w-0 rounded-[5px] px-2 py-1.5" style={inputStyle} /></label><label className="flex min-w-0 flex-col gap-1 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>结束<input type="date" aria-label={`限额 ${index + 1} 结束`} value={drafts[limit.limitId]?.end ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [limit.limitId]: { ...current[limit.limitId], end: event.target.value } }))} className="font-num min-w-0 rounded-[5px] px-2 py-1.5" style={inputStyle} /></label><label className="flex min-w-0 flex-col gap-1 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>金额<input inputMode="decimal" aria-label={`限额 ${index + 1} 金额`} value={drafts[limit.limitId]?.amount ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [limit.limitId]: { ...current[limit.limitId], amount: event.target.value.replace(/[^0-9.]/g, '') } }))} className="font-num min-w-0 rounded-[5px] px-2 py-1.5 text-right" style={inputStyle} /></label><button type="button" aria-label={`保存限额 ${index + 1}`} disabled={updateMutation.isPending} onClick={() => void update(limit)} className="justify-self-end rounded-[5px] px-2 py-1.5 text-[11.5px] disabled:opacity-50" style={{ color: 'var(--g-accent)' }}>保存</button></div>)}
        {limits.length > 0 && <div className="text-[11px]" style={{ color: 'var(--g-ink-2)' }}>{rangeLabel}</div>}
        <div className="grid grid-cols-1 items-end gap-2 border-t pt-3 sm:grid-cols-[1fr_1fr_90px_110px_32px]" style={{ borderColor: 'var(--g-border)' }}><label className="flex min-w-0 flex-col gap-1 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>新限额开始<input type="date" value={newStart} onChange={(event) => setNewStart(event.target.value)} className="font-num min-w-0 rounded-[5px] px-2 py-1.5" style={inputStyle} /></label><label className="flex min-w-0 flex-col gap-1 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>结束<input type="date" value={newEnd} onChange={(event) => setNewEnd(event.target.value)} className="font-num min-w-0 rounded-[5px] px-2 py-1.5" style={inputStyle} /></label><label className="flex min-w-0 flex-col gap-1 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>币种<select value={newCurrencyCode} onChange={(event) => setNewCurrencyCode(event.target.value)} className="min-w-0 rounded-[5px] px-2 py-1.5" style={inputStyle}>{(currenciesQuery.data?.data ?? []).filter((currency) => currency.attributes.enabled !== false).map((currency) => <option key={currency.id} value={currency.attributes.code}>{currency.attributes.code}</option>)}</select></label><label className="flex min-w-0 flex-col gap-1 text-[11px]" style={{ color: 'var(--g-ink-2)' }}>金额<input inputMode="decimal" value={newAmount} onChange={(event) => setNewAmount(event.target.value.replace(/[^0-9.]/g, ''))} className="font-num min-w-0 rounded-[5px] px-2 py-1.5 text-right" style={inputStyle} /></label><button type="button" title="添加限额" aria-label="添加限额" disabled={createMutation.isPending} onClick={() => void create()} className="justify-self-end rounded p-1.5 disabled:opacity-50" style={{ color: 'var(--g-accent)' }}><Plus size={14} /></button></div>
      </div>
    </Modal>
    <Modal open={editOpen} onClose={() => setEditOpen(false)} title="编辑预算" width={420} footer={<>
      <button type="button" title="删除预算" aria-label={`删除预算 ${attrs.name}`} disabled={deleteBudgetMutation.isPending} onClick={() => void removeBudget()} className="mr-auto rounded p-1.5 disabled:opacity-50" style={{ color: 'var(--g-danger)' }}><Trash2 size={14} /></button>
      <button type="button" disabled={updateBudgetMutation.isPending} onClick={() => setEditOpen(false)} className="rounded-[6px] px-3 py-1.5 text-[12px]" style={{ color: 'var(--g-ink-2)' }}>取消</button>
      <button type="button" disabled={updateBudgetMutation.isPending} onClick={() => void saveBudget()} className="rounded-[6px] px-3 py-1.5 text-[12px] disabled:opacity-50" style={{ background: 'var(--g-accent)', color: 'var(--g-accent-ink)' }}>{updateBudgetMutation.isPending ? '保存中…' : '保存'}</button>
    </>}>
      <div className="flex flex-col gap-3">
        <label className="flex flex-col gap-1 text-[12px]" style={{ color: 'var(--g-ink-2)' }}>名称<input autoFocus value={editName} onChange={(event) => setEditName(event.target.value)} className="rounded-[6px] px-2.5 py-1.5" style={inputStyle} /></label>
        <label className="flex items-center gap-2 text-[12px]" style={{ color: 'var(--g-ink)' }}><input type="checkbox" checked={editActive} onChange={(event) => setEditActive(event.target.checked)} />启用预算</label>
      </div>
    </Modal>
  </>
}
