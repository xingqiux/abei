...
const inputStyle = { background: 'light-dark(var(--color-gray-100), var(--color-gray-700))', color: 'light-dark(var(--color-gray-900), var(--color-gray-100))', border: '1px solid light-dark(var(--color-gray-200), var(--color-gray-600))' } as const
...
export function BudgetRow({ budget, limits, range, limitsLoading = false, limitsError = false, onRetryLimits }: { budget: Budget; limits: BudgetLimitInfo[]; range: DateRange; limitsLoading?: boolean; limitsError?: boolean; onRetryLimits?: () => void }) {
  ...
  return <>
    <div className="group flex min-h-8 flex-wrap items-center gap-2 rounded-[4px] px-2 py-1 text-[12.5px] hover:bg-[light-dark(var(--color-gray-100), var(--color-gray-700))] sm:flex-nowrap sm:gap-3">
      <div className="min-w-0 flex-1 truncate text-gray-900 dark:text-gray-100">{attrs.name}</div>
      {limitsLoading ? <div className="order-2 w-full text-left text-[11px] sm:order-none sm:w-[350px] sm:text-right text-gray-500 dark:text-gray-400">限额加载中…</div> : limitsError ? <div className="order-2 flex w-full items-center gap-2 text-[11px] sm:order-none sm:w-[350px] sm:justify-end text-red-600 dark:text-red-400"><span>限额加载失败</span><button type="button" onClick={onRetryLimits} style={{ color: 'light-dark(var(--color-indigo-600), var(--color-indigo-500))' }}>重试</button></div> : <div className="order-2 flex w-full shrink-0 flex-col gap-1 sm:order-none sm:w-[350px]">{currencySummaries.map((summary) => <div key={summary.code || summary.symbol} className="flex min-w-0 items-center gap-3"><div className="flex min-w-[90px] flex-1 items-center sm:w-[150px] sm:flex-none">{summary.hasLimit ? <ProgressBar pct={summary.pct} colorVar={summary.over ? 'light-dark(var(--color-red-600), var(--color-red-400))' : 'light-dark(var(--color-indigo-600), var(--color-indigo-500))'} /> : <span className="text-[11px] text-gray-500 dark:text-gray-400">未设限额</span>}</div><div className="font-mono tabular-nums min-w-0 flex-1 text-right sm:w-[187px] sm:flex-none" title={summary.code} style={{ color: summary.over ? 'light-dark(var(--color-red-600), var(--color-red-400))' : 'light-dark(var(--color-gray-900), var(--color-gray-100))' }}>{summary.symbol}{formatAmount(summary.spent)}{summary.hasLimit && <span style={{ color: 'light-dark(var(--color-gray-500), var(--color-gray-400))' }}> / {summary.symbol}{formatAmount(summary.total)}</span>}</div></div>)}</div>}
