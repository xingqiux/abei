...
import { completeGoogleMailboxOAuth } from '../../api/mailbox'
...
export function GoogleOAuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    void completeGoogleMailboxOAuth({ code, state })
      .then((settings) => {
        queryClient.setQueryData(['mailbox-settings'], settings)
        showToast({ kind: 'success', message: `Google 邮箱 ${settings.data.attributes.email} 已连接` })
        return navigate({ to: '/mail', replace: true })
      })
      .catch((cause: unknown) => {
    ...
    <div className="mx-auto flex min-h-72 max-w-md flex-col items-center justify-center gap-4 text-center">
      {failure ? (
        <>
          ...
          <Button variant="secondary" size="md" onClick={() => void navigate({ to: '/mail' })}>
            返回邮件工作台
          </Button>
        </>
      ) : (
