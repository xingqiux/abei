...
/** 按阿贝规范格式化带符号金额：支出 -¥1,234.56 / 收入 +¥1,234.56 / 转账 ¥1,234.56 */
...
/** "2026年8月7日 周四"，单日选择器的显示格式 */
export function formatDateLabel(isoDate: string): string {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(isoDate.slice(0, 10))) return '选择日期'
  const d = parseDateOnlyLocal(isoDate)
  return `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日 周${weekdayLabels[d.getDay()]}`
}
...
/**
 * "YYYY-MM-DD HH:mm"，输入带时区偏移（如 +08:00）的完整 ISO 时间戳。
 * 年份不能省：token「签发于 08-06 · 到期 08-06」看着像签发即到期，实际差一年。
 */
