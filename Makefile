...
.PHONY: help bootstrap up down clean reset ps logs build test test-backend test-cli test-web test-e2e test-empty-start lint analyze-backend audit migrate shell config release
...
clean:
	@status=0; \
		$(COMPOSE) --profile test --profile e2e down -v --remove-orphans || status=$$?; \
...
test: test-backend test-cli test-web
...
config: .env
	$(COMPOSE) --profile test --profile e2e config --quiet
