...
help:
	...
	@echo "up          起 4 个容器：db mail app abaku-web"
	@echo "down        停本地容器"
	@echo "logs        跟随 app 与 abaku-web 日志"
	...
	@echo "build-image 构建 app 与 abaku-web 镜像"

.env:
	cp .env.example .env

up: .env
	$(COMPOSE) up -d --build --wait db mail app abaku-web

down:
	$(COMPOSE) down --remove-orphans

logs:
	$(COMPOSE) logs -f app abaku-web
