...
（2026-08-05 已修：挑战要等解压真的过了才 consumed，密码错了退回 `needs_secret`
继续开着，`attempts` 上限 5 次，用完标 exhausted、任务转 failed。）

## 七、动工顺序

1. ~~**加 `suggested_by` 列**（迁移 + `PATCH` 的 `as_suggestion` 参数 + review 输出带上它）。
   先做这个，因为它是数据结构，越晚做代价越大。~~ 2026-08-05 完成。
2. ~~**密码挑战加失败保护**（尝试上限、错了不算 consumed）。~~ 2026-08-05 完成。
3. ~~**`ffc mcp`**：把上面 8 个工具接出来，跑通「列任务 → 拿 review → 改一行 → 干跑」。~~
   2026-08-06 完成；Web Agent 共用同一能力注册表，正式导入和密码走独立人工审批。
