...
阿贝的界面。React 19 + TypeScript + Vite；账本业务走同域 `/v1` 打 `abei-api`，
未建模的 Firefly 接口经 `/v1/firefly` 逃生舱透传；财务助手通过同域 `/api/ai` 调可信 `abei-agent`。
...
```bash
npm install
npm run dev        # http://localhost:5173，Vite 分别代理 abei-api 和 abei-agent
```

proxy 目标写在 `vite.config.ts`：`/v1` 和 `/health` 到 abei-api 的 18002，`/api/ai` 到 abei-agent 的 18003。
Firefly 自己的 18001 不再由浏览器直连，账本请求一律经 abei-api。
...
src/
  ...
  components/
    ...
    abei/      业务件：Modal、Combobox、TransactionRow、图表、Toast、空态错误态
