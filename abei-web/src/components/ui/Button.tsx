...
/**
 * 按钮的类名配方。单独导出是给「长得像按钮但不是 button」的元素用——
 * 主要是路由 `Link`：它得是 `<a>` 才能中键新开、才能被读屏当链接播报，
 * 但视觉上必须和旁边的按钮一模一样，抄一遍类名迟早会抄漏。
 */
export function buttonClass({
  ...
  return `inline-flex items-center justify-center rounded-md font-semibold whitespace-nowrap transition-colors disabled:pointer-events-none disabled:opacity-50 ${SIZES[size]} ${VARIANTS[variant]} ${block ? 'w-full' : ''} ${className}`
}
