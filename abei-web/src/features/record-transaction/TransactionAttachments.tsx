...
import { DownloadSimple, Paperclip, PencilSimple, Trash } from '@phosphor-icons/react'
import {
...
export function TransactionAttachments({ groupId, journalId }: { groupId: string; journalId: string }) {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      showToast({ kind: 'error', message: error instanceof AbeiApiError ? error.message : '附件上传失败', duration: 6000 })
    }
  }
  ...
  async function download(id: string, fallbackName: string) {
    try {
      ...
      showToast({ kind: 'error', message: error instanceof AbeiApiError ? error.message : '附件下载失败', duration: 6000 })
    }
  }
  ...
  async function saveMetadata() {
    ...
    try {
      ...
      showToast({ kind: 'error', message: error instanceof AbeiApiError ? error.message : '附件更新失败' })
    }
  }
  ...
  async function remove() {
    ...
    try {
      ...
      showToast({ kind: 'error', message: error instanceof AbeiApiError ? error.message : '附件删除失败' })
    }
  }
      ...
      <section className="mt-3 border-t border-[var(--border-subtle)] pt-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          <span className="flex items-center gap-1.5 text-[11.5px] text-[var(--text-secondary)]">
            <Paperclip aria-hidden className="size-3.5" />附件
          </span>
        ...
        {query.isLoading ? (
          ...
          <InlineError message="附件加载失败" error={query.error} onRetry={() => void query.refetch()} />
            ...
            {attachments.map((attachment) => {
              ...
              return (
                <li key={attachment.id} className="flex min-h-7 items-center gap-1 rounded px-1.5 hover:bg-[var(--surface-hover)]">
                  ...
                  <span className="num text-[10.5px] text-[var(--text-secondary)]">
