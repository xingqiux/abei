...
export function ExportPanel() {
  ...
  async function run() {
    ...
    try {
      ...
      showToast({ kind: 'error', message: error instanceof AbeiApiError ? error.message : 'CSV 导出失败', duration: 6000 })
    } finally {
      setPending(false)
    }
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {type === 'transactions' && (
        accounts.isError ? (
          <InlineError message="账户加载失败" error={accounts.error} onRetry={() => void accounts.refetch()} />
        ) : (
