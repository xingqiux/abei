...
beforeEach(() => {
  mocks.apiGet.mockReset().mockResolvedValue({ data: [recurrence({ id: '5', title: 'Netflix' })] })
  mocks.apiPost.mockReset().mockResolvedValue(triggeredResponse())
...
describe('SubscriptionsTab 列表', () => {
  ...
  it('没配账户模板时标出来而不是显示空白', async () => {
    mocks.apiGet.mockResolvedValue({ data: [recurrence({ id: '8', title: '裸订阅', tx: null })] })
    renderTab()
  ...
  it('一条都没有时给引导空态', async () => {
    ...
    expect(await screen.findByText('没有定期交易。在 Firefly III 中创建 recurrence 后可在此记账')).toBeInTheDocument()
  })
...
describe('SubscriptionsTab 记这一笔', () => {
  it('走 POST /v1/firefly/api/v1/recurrences/{id}/trigger，带上今天的日期', async () => {
    renderTab()
    fireEvent.click(await screen.findByRole('button', { name: '记这一笔' }))

    await waitFor(() => expect(mocks.apiPost).toHaveBeenCalledWith(
      `/v1/firefly/api/v1/recurrences/5/trigger?date=${TODAY}`,
      {},
    ))
  })
  ...
  it('200 但没生成交易时算失败：不显示已记，提示可能已记过', async () => {
    ...
    await waitFor(() => expect(mocks.toast).toHaveBeenCalledWith(
      expect.objectContaining({ kind: 'error', message: expect.stringContaining('未生成交易') }),
    ))
...
describe('SubscriptionsTab 防重复触发', () => {
  ...
  it('停用的定期交易不给手动触发', async () => {
    mocks.apiGet.mockResolvedValue({ data: [recurrence({ id: '5', title: 'Netflix', active: false })] })
    renderTab()
...
describe('SubscriptionsTab 触发失败', () => {
  it('提示结果未知让人去刷新确认，绝不给「重试」按钮', async () => {
    mocks.apiPost.mockRejectedValue(new Error('timeout'))
    renderTab()
