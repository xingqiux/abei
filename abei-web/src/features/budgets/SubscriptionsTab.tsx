...
export function SubscriptionsTab() {
  ...
  if (recurrences.isError) {
    return <ErrorState message="订阅加载失败" error={recurrences.error} onRetry={() => void recurrences.refetch()} />
  }
  const list = recurrences.data?.data ?? []
  if (list.length === 0) {
    return (
      <EmptyState
        message="没有定期交易。在 Firefly III 中创建 recurrence 后可在此记账"
        action={{ label: '记一笔', onClick: openRecordForm }}
      />
    )
  }
