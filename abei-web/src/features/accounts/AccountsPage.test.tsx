...
function stubAccounts(list: Account[]) {
  mocks.apiGet.mockImplementation((path: string) => {
    // 列表已经建模成 abei-api 的资源；单个账户还没有 accounts.show，走逃生舱。
    if (path === '/v1/accounts') return Promise.resolve(accountsPage(list))
    const match = /^\/v1\/firefly\/api\/v1\/accounts\/(\d+)$/.exec(path)
    if (match) {
...
beforeEach(() => {
  ...
  mocks.apiPut.mockReset().mockImplementation((path: string) => {
...
describe('AccountsPage 列表', () => {
  it('列出账户名、余额和总数', async () => {
    ...
    expect(mocks.apiGet).toHaveBeenCalledWith('/v1/accounts', { type: 'asset', limit: 40, page: 1 })
  })

  it('资产合计按币种汇总已加载的资产账户', async () => {
    renderPage()
  ...
  it('资产合计跟着「显示已归档」走，不含列表里没有的账户', async () => {
    stubAccounts([CHECKING, account({ id: '3', name: '旧卡', current_balance: '500.00', active: false })])
    renderPage()
    await screen.findByText('招行储蓄卡')

    // 列表里看不到「旧卡」，合计里也不该有它
    expect(screen.queryByText('旧卡')).not.toBeInTheDocument()
    expect(screen.getByText(/不含已归档账户/)).toBeInTheDocument()
    expect(screen.queryByText('¥1,734.50')).not.toBeInTheDocument()

    fireEvent.click(screen.getByLabelText('显示已归档'))

    expect(screen.getByText('¥1,734.50')).toBeInTheDocument()
    expect(screen.getByText(/(^|\s)含已归档账户/)).toBeInTheDocument()
  })
...
describe('AccountsPage 归档', () => {
  ...
  it('归档失败时报错，不谎报成功', async () => {
    mocks.apiPut.mockReset().mockRejectedValue(new Error('boom'))
    renderPage()
    await screen.findByText('招行储蓄卡')
...
describe('AccountsPage 分类页签', () => {
  // 页签叫 liabilities，abei-api 认的是 liability。两者在 Firefly 里指同一组账户类型
  // （Loan / Debt / Mortgage / CreditCard），换个写法不改结果。
  it('切到负债页签按新类型重新拉数据', async () => {
    ...
    await waitFor(() => expect(mocks.apiGet).toHaveBeenCalledWith(
      '/v1/accounts',
      { type: 'liability', limit: 40, page: 1 },
    ))
  })
