...
/** 交易页的特殊视图。目前只有一个：未分类回填审阅（AI 建议 → 人确认）。 */
export type TransactionsView = 'uncategorized'

/**
 * 交易页的搜索参数。
 *
 * 这里手写接口而不是 `ReturnType<typeof validateTransactionSearch>`：TanStack Router
 * 拿它当 `<Link to="/transactions">` 的入参类型，推导出来的形状里每个字段都是必填
 * （值可以是 undefined，但键不能少），于是新加一个 view 就会逼着所有旧调用点补写。
 * 标成可选属性才不会踩这个坑。
 */
...
export function validateTransactionSearch(search: Record<string, unknown>): TransactionSearch {
  ...
  return {
    ...
    view: search.view === 'uncategorized' ? 'uncategorized' : undefined,
  }
}
